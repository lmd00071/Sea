...
- (instancetype)initWithFrame:(CGRect)frame videoURLStr:(NSString *)videoURLStr image:(UIImage *)image{
    self = [super init];
    if (self) {
        self.frame = frame;
        self.backgroundColor = [UIColor blackColor];
        
        self.palyImageView=[[UIImageView alloc]init];
        self.palyImageView.image=image;
        [self addSubview:_palyImageView];
        [self.palyImageView mas_makeConstraints:^(MASConstraintMaker *make) {
...
//        self.playerLayer.videoGravity = AVLayerVideoGravityResize;
        ...
        [self.fullScreenBtn setTitle:@"返回" forState:UIControlStateNormal];
        self.fullScreenBtn.titleLabel.font = [UIFont systemFontOfSize:14];
