...
- (void)awakeFromNib
{
    ...
    [self.progressSlider setThumbImage:[UIImage imageNamed:@"slid.png"] forState:UIControlStateNormal];
...
//    [self.progressSlider setMinimumTrackImage:[UIImage imageNamed:@"MinimumTrackImage"] forState:UIControlStateNormal];
    ...
    //[self.playOrPauseBtn setImage:[UIImage imageNamed:@"pase1.png"] forState:UIControlStateNormal];
    // 设置按钮的状态
...
#pragma mark - 设置播放的视频
- (void)setUrlModel:(NSString *)fileStr
{

    //处理文件名
    NSRange range1={fileStr.length-4,4};
    NSRange range2={0,fileStr.length-4};
    
    NSString *vedioTou=[fileStr substringWithRange:range2];
    NSString *vedioWei=[fileStr substringWithRange:range1];
    

    NSString *cachePath = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Private Documents/Cache"];
      NSURL *url=[NSURL fileURLWithPath:[cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%ld%@",[vedioTou hash],vedioWei]]];
...
- (IBAction)playOrPause:(UIButton *)sender {
    ...
    if (sender.selected) {
        ...
        //[self.playOrPauseBtn setImage:[UIImage imageNamed:@"play1.png"] forState:UIControlStateNormal];
        [self.player pause];
        [self removeShowTimer];
        [self removeProgressTimer];
